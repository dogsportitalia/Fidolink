/**
 * Script di importazione dati da PostgreSQL (JSON) a SQLite
 * Uso: tsx script/import-data.ts
 */
...
const pageVisits = [
  { id: 1,   date: "2026-02-03", count: 6 },
  { id: 7,   date: "2026-02-04", count: 7 },
  { id: 14,  date: "2026-02-05", count: 9 },
  { id: 23,  date: "2026-02-06", count: 2 },
  { id: 25,  date: "2026-02-07", count: 9 },
  { id: 34,  date: "2026-02-08", count: 4 },
  { id: 38,  date: "2026-02-09", count: 6 },
  { id: 44,  date: "2026-02-10", count: 3 },
  { id: 47,  date: "2026-02-11", count: 3 },
  { id: 50,  date: "2026-02-12", count: 4 },
  { id: 54,  date: "2026-02-13", count: 2 },
  { id: 56,  date: "2026-02-14", count: 5 },
  { id: 61,  date: "2026-02-15", count: 2 },
  { id: 63,  date: "2026-02-16", count: 4 },
  { id: 67,  date: "2026-02-17", count: 2 },
  { id: 69,  date: "2026-02-18", count: 6 },
  { id: 75,  date: "2026-02-19", count: 2 },
  { id: 77,  date: "2026-02-20", count: 5 },
  { id: 82,  date: "2026-02-22", count: 2 },
  { id: 84,  date: "2026-02-24", count: 2 },
  { id: 86,  date: "2026-02-25", count: 4 },
  { id: 90,  date: "2026-02-26", count: 2 },
  { id: 92,  date: "2026-02-27", count: 2 },
  { id: 94,  date: "2026-02-28", count: 2 },
  { id: 96,  date: "2026-03-02", count: 1 },
  { id: 97,  date: "2026-03-04", count: 2 },
  { id: 99,  date: "2026-03-05", count: 1 },
  { id: 100, date: "2026-03-06", count: 1 },
  { id: 101, date: "2026-03-07", count: 2 },
  { id: 103, date: "2026-03-08", count: 2 },
...
sqlite.pragma("foreign_keys = OFF"); // disabilita FK durante import per evitare dipendenze ordine

const run = sqlite.transaction(() => {
  // Users
  const insUser = sqlite.prepare(`
    INSERT OR REPLACE INTO users (id, email, password_hash, reset_token, reset_token_expiry, created_at)
    VALUES (@id, @email, @passwordHash, @resetToken, @resetTokenExpiry, @createdAt)
  `);
  for (const u of users) {
    insUser.run({ id: u.id, email: u.email, passwordHash: u.password_hash, resetToken: u.reset_token, resetTokenExpiry: toUnix(u.reset_token_expiry), createdAt: toUnix(u.created_at) });
  }
  console.log(`✓ ${users.length} utenti importati`);

  // Tags
  const insTag = sqlite.prepare(`
    INSERT OR REPLACE INTO tags (id, public_id, claim_code_hash, owner_id, created_at, claimed_at, expires_at)
    VALUES (@id, @publicId, @claimCodeHash, @ownerId, @createdAt, @claimedAt, @expiresAt)
  `);
  for (const t of tags) {
    insTag.run({ id: t.id, publicId: t.public_id, claimCodeHash: t.claim_code_hash, ownerId: t.owner_id, createdAt: toUnix(t.created_at), claimedAt: toUnix(t.claimed_at), expiresAt: toUnix(t.expires_at) });
  }
  console.log(`✓ ${tags.length} tag importati`);

  // Dog profiles
  const insProfile = sqlite.prepare(`
    INSERT OR REPLACE INTO dog_profiles (id, tag_id, name, photo_url, breed, sex, size, birthdate, medical_notes, instructions_text, contact_phone, contact_whatsapp, contact_email, show_phone, show_whatsapp, show_email, notify_on_scan, city, updated_at)
    VALUES (@id, @tagId, @name, @photoUrl, @breed, @sex, @size, @birthdate, @medicalNotes, @instructionsText, @contactPhone, @contactWhatsapp, @contactEmail, @showPhone, @showWhatsapp, @showEmail, @notifyOnScan, @city, @updatedAt)
  `);
  for (const p of dogProfiles) {
    insProfile.run({ id: p.id, tagId: p.tag_id, name: p.name, photoUrl: p.photo_url || null, breed: p.breed || null, sex: p.sex || null, size: p.size || null, birthdate: p.birthdate || null, medicalNotes: p.medical_notes || null, instructionsText: p.instructions_text || null, contactPhone: p.contact_phone || null, contactWhatsapp: p.contact_whatsapp || null, contactEmail: p.contact_email || null, showPhone: bool(p.show_phone), showWhatsapp: bool(p.show_whatsapp), showEmail: bool(p.show_email), notifyOnScan: bool(p.notify_on_scan), city: p.city || null, updatedAt: toUnix(p.updated_at) });
  }
  console.log(`✓ ${dogProfiles.length} profili cane importati`);

  // Scan events
  const insScan = sqlite.prepare(`
    INSERT OR REPLACE INTO scan_events (id, tag_id, created_at, ip_hash, user_agent, latitude, longitude)
    VALUES (@id, @tagId, @createdAt, @ipHash, @userAgent, @latitude, @longitude)
  `);
  for (const s of scanEvents) {
    insScan.run({ id: s.id, tagId: s.tag_id, createdAt: toUnix(s.created_at), ipHash: s.ip_hash, userAgent: s.user_agent, latitude: s.latitude, longitude: s.longitude });
  }
  console.log(`✓ ${scanEvents.length} scan events importati`);

  // Page visits
  const insVisit = sqlite.prepare(`
    INSERT OR REPLACE INTO page_visits (id, date, count)
    VALUES (@id, @date, @count)
  `);
  for (const v of pageVisits) {
    insVisit.run(v);
  }
  console.log(`✓ ${pageVisits.length} page visits importati`);

  // Location links
  const insLink = sqlite.prepare(`
    INSERT OR REPLACE INTO location_links (id, token, latitude, longitude, dog_name, created_at, expires_at)
    VALUES (@id, @token, @latitude, @longitude, @dogName, @createdAt, @expiresAt)
  `);
  for (const l of locationLinks) {
    insLink.run({ id: l.id, token: l.token, latitude: l.latitude, longitude: l.longitude, dogName: l.dog_name, createdAt: toUnix(l.created_at), expiresAt: toUnix(l.expires_at) });
  }
  console.log(`✓ ${locationLinks.length} location links importati`);
});
