...
export async function sendWelcomeEmail(userEmail: string) {
  ...
  const legalUrl = process.env.APP_URL ? `${process.env.APP_URL}/legal` : 'https://fidolink.it/legal';

  const logoUrl = process.env.APP_URL ? `${process.env.APP_URL}/favicon.png` : 'https://fidolink.it/favicon.png';
...
export async function sendPasswordResetEmail(userEmail: string, resetToken: string) {
  ...
  const resetUrl = process.env.APP_URL 
    ...
    : `https://fidolink.it/reset-password?token=${resetToken}`;
